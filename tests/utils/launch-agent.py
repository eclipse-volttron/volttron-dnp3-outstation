...
if agent_identity:
    print(f"============ agent_identity {agent_identity}")
    ...
    if not os.path.exists(agent_keystore_dir):
        ...
        try:
            ...
            with open(keystore_file, 'w') as fout:
                fout.write(output)
            os.environ['AGENT_PUBLICKEY'] = json_obj.get('public')
            os.environ['AGENT_SECRETKEY'] = json_obj.get('secret')
        ...
        with open(keystore_file) as fp:
            ...
            os.environ['AGENT_PUBLICKEY'] = keypair.get('public')
            os.environ['AGENT_SECRETKEY'] = keypair.get('secret')
